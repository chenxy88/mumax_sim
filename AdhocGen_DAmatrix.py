...
def DA_matrix():
	...
	# ovf path
	mx3_output_path = r'D:\Skyrmions-data\FORC\DA fitting\st131\mx3 files'

	sim_batch_name = '33dFORC_st131'
	...
	params_sweep.Aex = [10, 12, 14, 16] #pJ/m
	params_sweep.Dind = [1.4, 1.6, 1.8, 2, 2.2] # mJ/M2
	ParamsSweepList = flatten(outer_product_object_list(params_sweep))

	for params in ParamsSweepList:

		sim_name = sim_batch_name + '_Aex_%g_Dind_%.2g'%(params.Aex, params.Dind)
		...
		Msat_var := 0.370757143*Mega  //Saturation magnetisation in A/m
		Ku1_var	:= 0.11816163*Mega  // Anistropy in J/m^3
		...
		FixDt = 1.000000E-13
		Temp = 850.000000
		temperature_run_time := 1.000000E-08		
		
		middle_layer := 6
		// save the middle slice of the config	
		AutoSave(CropLayer(m, middle_layer), 0.000000E+00) 
